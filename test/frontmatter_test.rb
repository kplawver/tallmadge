...
class FrontmatterTest < Minitest::Test
  def test_parse_simple_values
    meta, body = Tallmadge::Frontmatter.parse("---\nname: hello\ndescription: Says hi\n---\nBody text\n")
  ...
  def test_parse_quoted_values
    meta, = Tallmadge::Frontmatter.parse("---\nname: \"hello\"\ndescription: 'quoted'\n---\nx\n")
    assert_equal "hello", meta["name"]
  ...
  def test_no_frontmatter
    meta, body = Tallmadge::Frontmatter.parse("just text")
    assert_equal({}, meta)
    assert_equal "just text", body
  end

  def test_serialize_sorts_keys_alphabetically
    text = Tallmadge::Frontmatter.serialize({ "name" => "x", "description" => "y" }, "body")
    assert_equal "---\ndescription: y\nname: x\n---\n\nbody", text
  end

  def test_serialize_joins_arrays
    text = Tallmadge::Frontmatter.serialize({ "tags" => %w[a b] }, "")
  ...
  def test_yaml_fallback_parses_real_yaml_skill_md
    sample = <<~MD
      ...
      Body here
    MD
    meta, body = Tallmadge::Frontmatter.parse(sample)
  ...
  def test_roundtrip
    text = Tallmadge::Frontmatter.serialize({ "kind" => "task", "enabled" => true }, "prompt")
    meta, body = Tallmadge::Frontmatter.parse(text)
